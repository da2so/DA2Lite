            ...
            if layer_type == 'Conv':
                ...
                if self.clustering_cfg.CLUSTER not in act_based_clustering:
                    ...
                    if i_node['group'] in group_frequency:
                        ...
                        f_maps = self.activations[i_node['name']]
                        cluster_idx = self.criteria_class.get_cluster_idx(i_node=i_node,
                                                    f_maps=f_maps,
                                                    device=self.device)
        ...
        def save_fmaps(key):
            def forward_hook(module, inputs, outputs):
                if key not in self.activations:
                    ...
                    self.activations[key] = torch.cat((self.activations[key], outputs), dim=0)
            return forward_hook
        ...
        for name, layer in self.model.named_modules():
            ...
            if layer_type == 'BN':
                ...
                for i_link_conv in link_convs:
                    ...
                    self.hook_layers.append(layer.register_forward_hook(save_fmaps(i_link_conv)))    
